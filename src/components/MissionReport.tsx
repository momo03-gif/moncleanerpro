...
export default function MissionReport({ missionId, mode, userId, defaultOpen }: Props) {
...
function ViewerBody({ report }: { report: Report | null }) {
  ...
  return (
    <div className="space-y-3 text-sm" style={{ color: '#4A443D' }}>
      ...
      {report.issues && <Block title="Problème / dégât" accent="#B85A50"><p>{report.issues}</p></Block>}
      {report.lostFound && <Block title="Objet oublié" accent="#3E63DD"><p>{report.lostFound}</p></Block>}
